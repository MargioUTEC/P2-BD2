...
Para abordar este proyecto, nos hemos centrado en el **dominio musical**, un escenario ideal que combina dos mundos: el lenguaje humano (las letras) y las señales acústicas (el sonido).
...
Las bases de datos relacionales tradicionales son herramientas perfectas para búsquedas deterministas (ej. *"Dame la canción con ID 404"*). Sin embargo, fallan ante consultas humanas y difusas, como *"búscame canciones que hablen de amor en tiempos de guerra"* o *"encuéntrame algo que suene parecido a esta pista"*.
...
Para representar cada archivo de audio de forma numérica y permitir su análisis, se aplicó el método MFCC (Mel-Frequency Cepstral Coefficients).

MFCC es una técnica usada en procesamiento de audio porque captura cómo percibimos el sonido los humanos. Este método transforma la señal original en un conjunto de coeficientes que representan: El timbre del sonido, forma espectral y patrones relevantes para tareas de clasificación, búsqueda o similitud.
...
El frontend inicialmente fue desarrollado con Qt de Python, priorizando la usabilidad y la interacción intuitiva con los motores de búsqueda textual y multimedia implementados en el backend. La interfaz cuenta con **3 botones** independientes: una destinada a la recuperación basada en texto,otra orientada a la búsqueda de audios y la ultima para explotar la metada. En ambos casos, la aplicación ofrece elementos interactivos que permiten ejecutar querys de manera dinámica. En la ventana de texto, el usuario puede ingresar una consulta libre en un cuadro de texto, seleccionar el valor de Top-K. Tras la ejecución, los resultados son presentados en un panel que muestra los outputs en lo que se refleja la metadata y la información relevante de cada documento recuperado.
...
Para levantar la aplicación se deben iniciar primero los servicios de backend (APIs) y luego el servidor del frontend. La ejecución recomendada es en el siguiente orden, desde la carpeta raíz del proyecto:
...
En esta parte quisimos ver qué tan rápido es nuestro buscador comparado con PostgreSQL. Hicimos varias pruebas aumentando la cantidad de canciones (N) desde 2000 hasta 32000, para ver si el sistema aguantaba.
...
Básicamente, **GIN resultó ser 160 veces más rápido** y como en este proyecto nos interesa que el usuario encuentre la canción al instante (y no vamos a estar cambiando las letras de las canciones a cada rato), decidimos quedarnos con GIN para la arquitectura final.
...
1. Construimos y levantamos los servicios:
   ...
   ```
2. Accede al frontend en [http://localhost:8001](http://localhost:8001). Todas las peticiones del navegador se redirigen al backend interno publicado en el puerto 8000.


Los contenedores creados son:

- `proyecto2_backend`: contiene las rutas `/api` (audio/metadata) y `/api2` (texto).
...
### Backends-FastAPI
* api/: esta es la API para audio, metadata y consultas híbridas; monta los routers específicos  /audio, /metadata y /fusion, más un endpoint raíz de estado.

* api2/: API separada para búsquedas textuales (índice invertido), montada con /text y un endpoint raíz.

### Frontend-FastAPI/Jinja

* frontend/ aplica como nuestro formato visual web donde monta los 2 backends anteriores y entrega la página principal con la UI de búsquedas.

* ./templates/ y ./static/ contienen la plantilla HTML principal y recursos JS/CSS.

### Procesamiento de texto (índice invertido propio)

* Indice_invertido/: implementación local del índice invertido y utilidades; el archivo invertido_indice.py carga el dataset de letras, stoplist y construye el índice con preprocesamiento (tokenización, stemming, TF-IDF).

### Procesamiento de audio y metadata

* audio/: scripts para el pipeline acústico y metadatos donde incluye codebooks MFCC (audio/codebook/) y utilidades para consultar la base de metadata en SQLite, como metadata/build_metadata_sqlite.py.

### Otros recursos

* images/ : guarda capturas de la UI referenciadas en el README.

* Testers y experimentación (p.ej., prueba_audio.py, prueba_texto_gin.py, experimentacion/)

* Archivos docker: facilita orquestar los servicios y desplegar el proyecto.
